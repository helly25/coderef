...
| `/path/to/file:label-name`            | **Named-region label**. Matches a `Label('label-name') ... EndLabel` region in the target file. Disambiguator: `:` followed by digits or `N-M` is a line/range; anything else is a label.                  |
| `:label-name`                         | **Same-file label shortcut** — resolves to a label in the *referencing* file. Useful for in-module coupled blocks.                                                                                         |
...
markers, pair them into blocks, and resolve ids (Shape B + C). In the
same pass, find `Label('name') ... EndLabel` pairs (§10.x) and build
the label index. Build:
...
- **Named-region labels** (§10 — `Label('name') ... EndLabel`, refs
  via `path:label-name`; same-file `:label-name` shortcut). Refactor
  survival is a v0.2 deliverable; line-range targets remain supported.
...
# Named-region label (refactor-stable; the range follows the content).
# IfChange
# Label('argon2-params')
